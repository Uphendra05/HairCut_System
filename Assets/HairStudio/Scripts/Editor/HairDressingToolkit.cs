...
namespace HairStudio
{
    public class HairDressingToolkit
    {
        ...
        public float toolRadius {
            get => EditorPrefs.GetFloat(TOOL_RADIUS_PREF_LABEL, 100);
            set => EditorPrefs.SetFloat(TOOL_RADIUS_PREF_LABEL, value);
        }

        public float toolStrength {
            get => EditorPrefs.GetFloat(TOOL_STRENGTH_PREF_LABEL, 0.5f);
            set => EditorPrefs.SetFloat(TOOL_STRENGTH_PREF_LABEL, value);
        }

        public int rootZone {
            get => EditorPrefs.GetInt(ROOT_ZONE_PREF_LABEL);
        ...
        public void CombGuide(Guide guide, Vector3 motion) {
            ...
            foreach (var seg in guide.segments) {
                ...
                guideChanged = true;
                seg.localPosition += dressing.transform.InverseTransformDirection(motion) * influence * dressing.inverseScaleFactor;
            }
            if (guideChanged) {
                // manage length
                foreach (var s in guide.segments) {
                    ...
                    toNext -= toNext.normalized * localLength * dressing.inverseScaleFactor;
        ...
        public void ChangeLength(Guide guide, bool grow) {
            if (grow) {
                ...
                while (guide.lastSegmentLength > guide.segmentLength * (1 + MINIMUM_LAST_LENGTH_RATIO)) {
                    ...
                    last.localPosition = last.previous.localPosition + direction * guide.segmentLength * dressing.inverseScaleFactor;
        ...
        private Guide GenerateGuide(Vector3 localRoot) {
            ...
            for (int j = 0; j < dressing.segmentCountPerGuide; j++) {
                ...
                if (previous == null) {
                    ...
                    offset *= dressing.inverseScaleFactor;
