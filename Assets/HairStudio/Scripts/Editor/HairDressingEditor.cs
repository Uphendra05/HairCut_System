...
namespace HairStudio
{
    ...
    public class HairDressingEditor : Editor
    {
        ...
        private void DrawCommonProperties() {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("alwaysDisplayRoots"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("alwaysDisplayGuides"));
        ...
        static void DisplayUnselected(HairDressing dressing, GizmoType gizmoType) {
            if(gizmoType == (GizmoType.Active | GizmoType.InSelectionHierarchy | GizmoType.Selected)) return;
            if (!dressing.alwaysDisplayRoots && !dressing.alwaysDisplayGuides) return;

            Handles.zTest = UnityEngine.Rendering.CompareFunction.Less;
            Handles.color = Color.grey;
            var points = new List<Vector3>();
            if (dressing.alwaysDisplayGuides) {
                foreach (var seg in dressing.guides.SelectMany(g => g.segments).Where(s => s.next != null)) {
                    points.Add(dressing.transform.TransformPoint(seg.localPosition));
                    points.Add(dressing.transform.TransformPoint(seg.next.localPosition));
                }
            }

            if (dressing.alwaysDisplayRoots) {
                foreach (var root in dressing.roots.Get()) {
                    var wRoot = dressing.transform.TransformPoint(root.LocalPos);
                    points.Add(wRoot);
                    points.Add(wRoot + (wRoot - dressing.gameObject.transform.position).normalized * dressing.rootRadius);
                }
            }
